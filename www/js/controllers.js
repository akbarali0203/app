...
$http.get(config.urlBase+'json/data_places_categories.php')
  .success(function(data, status, headers,config){
    ...
    $scope.categoriesPlaceNames = data.categories;
  })
  .error(function(data, status, headers,config){
    ...
    $scope.doLogin=function(){
      ...
      if(user_email && user_password){
          ...
          var str = config.urlBase+"controller/data_login-new.php";
          ...
          $http.post(str ,$scope.userLoginDetail, {
            headers : {
  ...
  var getFacebookProfileInfo = function (authResponse) {
    ...
    facebookConnectPlugin.api('/me?fields=id,name,gender,location,picture.type(large),email,birthday&access_token=' + authResponse.accessToken, null,
      function (response) {
        ...
        //adding Bydefault city Essen if the user city dont exist in our DB
        $scope.pl = $scope.categoriesPlaceNames.filter(Item =>  Item.place_category_name == location);
        if($scope.pl.length == 0)
        {
          location = 'Essen';
        }
...
//user_photo: 'https://romlove.com/sites/default/files/square.original.300x300uc.jpg', 
    $scope.saveusers = function (user){
      ...
      if(user.user_uniemail) {
        console.log(user.user_uniemail);
        var splitUniEmailAddress = user.user_uniemail.split("@");
        var uniEmailAddressIs = splitUniEmailAddress[1];
        console.log(uniEmailAddressIs);
        var checkEmailIsRite = config.urlBase+'/controller/uniemail.php?uniemail='+uniEmailAddressIs+'&fullEmail='+user.user_uniemail;
        ...
        .success(function(data, status, headers,config){
          ...
          if(data.status == 'exists'){
            $ionicLoading.show({ template: 'Überprüfe deine Eingabe. Deine Uni E-Mail existiert bereits.', noBackdrop: true, duration: 2000});
          }else if(data.status == 'error'){
              $ionicLoading.show({ template: 'Überprüfe deine Eingabe. Deine Uni E-Mail Adresse scheint nicht korrekt zu sein.', noBackdrop: true, duration: 2000});
          }else if(data.status == 'success'){
            $scope.uniname = data.uniname;
            $scope.loginAndSendWelcomeEmail($scope.user);
          }
...
$scope.loginAndSendWelcomeEmail = function(user) {
    ...
    if(response['data'].indexOf('request_status') == -1){
      ...
      .success(function(response){
             ...
             if($scope.uniname){
                ...
                $scope.sendEmailForVerification = {
                  userid : localStorage.getItem('id_user'),
                  uniEmail : $scope.user.user_uniemail,
                  old_uniemail : $scope.user.user_uniemail,
                };

                if(localStorage.getItem('id_user')){

                  var urlVerify = config.urlBase+"controller/sendEmailForVerification.php";
                  $http.post(urlVerify,$scope.sendEmailForVerification, {
                           headers : {
                           'Content-Type' : 'application/x-www-form-urlencoded; charset=UTF-8'
                           } } )
...
.controller('HomeCtrl', ['$rootScope', '$scope', '$http', '$state', 'config', 'strings', '$ionicPlatform', '$ionicPopup', '$ionicHistory', '$cordovaToast', '$localStorage', '$cordovaInAppBrowser', function($rootScope, $scope, $http, $state, config, strings, $ionicPlatform, $ionicPopup, $ionicHistory, $cordovaToast, $localStorage, $cordovaInAppBrowser) {
  ...
  $http.get(config.urlBase+'json/data_places_categories.php')
    .success(function(data, status, headers,config){
      ...
      if(localStorage.getItem("user_city") == '' || localStorage.getItem("user_city") == undefined){
        ...
        if($scope.cityDetail.length == 0) {
          $scope.user_city_name = 'Essen';
          $scope.user_city = 'essen.jpg';
          localStorage.setItem('user_city', $scope.user_city_name);
         } else {
...
.controller('UniIdCtrl', ['$scope', '$localStorage', '$state', '$location', '$ionicHistory', '$http', 'config', 'strings', '$ionicLoading', '$ionicModal', '$cordovaCamera', '$cordovaFile', '$cordovaFileTransfer', '$cordovaDevice', '$ionicPopup'  , function($scope, $localStorage, $state, $location, $ionicHistory, $http, config, strings, $ionicLoading, $ionicModal, $cordovaCamera, $cordovaFile, $cordovaFileTransfer, $cordovaDevice, $ionicPopup ) {
...
.success(function(data, status, headers,config){
    ...
    $scope.uuniemail = $scope.users[0].user_uniemail;

    if($scope.uuniemail == " "){
       $scope.users[0].user_uniemail = "";
    }
...
$ionicModal.fromTemplateUrl('editProfile.html', function($ionicModal) {
    $scope.share = $ionicModal;
}, {
    scope: $scope,
    animation: 'slide-in-up'
...
.controller('VerifyCtrl', ['$scope', '$localStorage', '$state', 'config', '$http', '$location', '$ionicPlatform', '$ionicHistory', '$ionicLoading', '$rootScope', '$ionicPopup', 'ClockSrv', function($scope, $localStorage, $state, config, $http, $location, $ionicPlatform, $ionicHistory, $ionicLoading, $rootScope, $ionicPopup, ClockSrv ) {
  ...
  $scope.checkUniName = function(v){
    ...
    $scope.msgVerify = false;
    $scope.msgVerifyError = false;
    $scope.msgVerifyErrorKorrekt = false;
  ...
  .success(function(data, status, headers,config){
           ...
           .success(function(){
                    ...
                    $ionicLoading.hide();
                    $scope.msgVerify = true;
                    $scope.msgVerifyError = false;
                    $scope.msgVerifyErrorKorrekt = false;

                    //$scope.msgVerify = "Bitte bestätige die Verifizierung und überprüfe auch deinen SPAM ordner."
                    $ionicLoading.show({ template: $scope.contactSuccess, noBackdrop: true, duration: 500 });
                    })
           ...
           .error(function(){
              ...
              $ionicLoading.hide();
              $scope.user.user_uniemail=lastemail;
              $scope.msgVerifyError = true;
              $scope.msgVerifyErrorKorrekt = false;
              $scope.msgVerify = false;
              //$scope.msgVerify = "Überprüfe deine Eingabe. Deine Uni E-Mail existiert bereits. \r Bei Problemen, wende dich an das UNIHELD Team: \r\n SUPPORT@UNIHELD:COM";
           }else {
            $ionicLoading.hide();
            $scope.user.user_uniemail=lastemail;
            $scope.msgVerifyErrorKorrekt = true;
            $scope.msgVerifyError =false;
            $scope.msgVerify = false;
            //$scope.msgVerifyError = "Überprüfe deine Eingabe. Deine Uni E-Mail Adresse scheint nicht korrekt zu sein.";
           }
          ...
          if(user.user_uniemail) {
            ...
            .success(function(data, status, headers,config){
              ...
              if(data.status == 'exists'){
                ...
                $scope.uninameCompeleteRest = data.uniname;
                $scope.loginAndSendWelcomeEmailFB(user);
              }
...
$scope.loginAndSendWelcomeEmailFB = function(user)
{
  var urlSignup = config.urlBase+'controller/new_user.php';
  $http.post(urlSignup, user, {
    headers : {
  ...
  .success(function(response){
    ...
    .success(function(response){
       ...
       // $ionicHistory.clearHistory();
        ...
        if($scope.uninameCompeleteRest){

          $scope.sendEmailForVerificationRest = {
            userid : localStorage.getItem('id_user'),
            uniEmail : $scope.admin.user_uniemail,
            old_uniemail : $scope.admin.user_uniemail,
          };

          if(localStorage.getItem('id_user')){

            var urlVerify = config.urlBase+"controller/sendEmailForVerification.php";
            $http.post(urlVerify,$scope.sendEmailForVerificationRest, {
                     headers : {
                     'Content-Type' : 'application/x-www-form-urlencoded; charset=UTF-8'
                     } } )
...
.controller('filterPlacesByCategoryCtrl', ['$scope', '$http', '$state', 'config', 'strings', '$ionicLoading', '$timeout', function($scope, $http, $state, config, strings, $ionicLoading, $timeout) {
