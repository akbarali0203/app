  ...
  ionic.Gestures.event = {
    ...
    collectEventData: function collectEventData(element, eventType, touches, ev) {
      ...
      return {
        ...
        /**
         * prevent the browser default Aktionen
...
(function () {
    ...
    var Swiper = function (container, params, _scope, $compile) {
        ...
        var defaults = {
            ...
            onLazyBildLoad: function (swiper, slide, image)
            onLazyBildReady: function (swiper, slide, image)
            */
        ...
        /*=========================
          Update on Bilds Ready
          ===========================*/
        s.imagesToLoad = [];
        s.imagesLoaded = 0;

        s.loadBild = function (imgElement, src, srcset, checkForComplete, callback) {
            var image;
            function onReady () {
        ...
        s.preloadBilds = function () {
            s.imagesToLoad = s.container.find('img');
            function _onReady() {
                if (typeof s === 'undefined' || s === null) return;
                if (s.imagesLoaded !== undefined) s.imagesLoaded++;
                if (s.imagesLoaded === s.imagesToLoad.length) {
                    if (s.params.updateOnBildsReady) s.update();
                    s.emit('onBildsReady', s);
                }
            }
            for (var i = 0; i < s.imagesToLoad.length; i++) {
                s.loadBild(s.imagesToLoad[i], (s.imagesToLoad[i].currentSrc || s.imagesToLoad[i].getAttribute('src')), (s.imagesToLoad[i].srcset || s.imagesToLoad[i].getAttribute('srcset')), true, _onReady);
            }
        };
        ...
        /*=========================
          ...
          ===========================*/
        s.lazy = {
            initialBildLoaded: false,
            loadBildInSlide: function (index, loadInDuplicate) {
                ...
                img.each(function () {
                    ...
                    s.loadBild(_img[0], (src || background), srcset, false, function () {
                        if (background) {
                        ...
                        if (s.params.loop && loadInDuplicate) {
                            ...
                            if (slide.hasClass(s.params.slideDuplicateClass)) {
                                ...
                                s.lazy.loadBildInSlide(originalSlide.index(), false);
                            }
                            else {
                                var duplicatedSlide = s.wrapper.children('.' + s.params.slideDuplicateClass + '[data-swiper-slide-index="' + slideOriginalIndex + '"]');
                                s.lazy.loadBildInSlide(duplicatedSlide.index(), false);
                            }
                        }
                        s.emit('onLazyBildReady', s, slide[0], _img[0]);
                    });

                    s.emit('onLazyBildLoad', s, slide[0], _img[0]);
                });
            ...
            load: function () {
                ...
                if (s.params.watchSlidesVisibility) {
                    s.wrapper.children('.' + s.params.slideVisibleClass).each(function () {
                        s.lazy.loadBildInSlide($(this).index());
                    });
                }
                else {
                    if (s.params.slidesPerView > 1) {
                        for (i = s.activeIndex; i < s.activeIndex + s.params.slidesPerView ; i++) {
                            if (s.slides[i]) s.lazy.loadBildInSlide(i);
                        }
                    }
                    else {
                        s.lazy.loadBildInSlide(s.activeIndex);
                    }
                }
                if (s.params.lazyLoadingInPrevNext) {
                    if (s.params.slidesPerView > 1) {
                        ...
                        if (nextSlide.length > 0) s.lazy.loadBildInSlide(nextSlide.index());

                        var prevSlide = s.wrapper.children('.' + s.params.slidePrevClass);
                        if (prevSlide.length > 0) s.lazy.loadBildInSlide(prevSlide.index());
                    }
                }
            },
            onTransitionStart: function () {
                if (s.params.lazyLoading) {
                    if (s.params.lazyLoadingOnTransitionStart || (!s.params.lazyLoadingOnTransitionStart && !s.lazy.initialBildLoaded)) {
                        s.lazy.load();
                    }
                }
            },
        ...
        s.init = function () {
            ...
            if (s.params.preloadBilds && !s.params.lazyLoading) {
                s.preloadBilds();
            }
