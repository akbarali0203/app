 ...
 *
 * ![Action Sheet](http://ionicframework.com.s3.amazonaws.com/docs/controllers/Aktionenheet.gif)
 *
 * @usage
 * To trigger an Action Sheet in your code, use the $ionicAktionenheet service in your angular controllers:
 *
 * ```js
 * angular.module('mySuperApp', ['ionic'])
 * .controller(function($scope, $ionicAktionenheet, $timeout) {
 *
 *  // Triggered on a button click, or some other target
 *  $scope.show = function() {
 *
 *    // Show the action sheet
 *    var hideSheet = $ionicAktionenheet.show({
 *      buttons: [
 *        { text: '<b>Share</b> This' },
 *        { text: 'Move' }
 *      ],
...
function($rootScope, $compile, $animate, $timeout, $ionicTemplateLoader, $ionicPlatform, $ionicBody, IONIC_BACK_PRIORITY) {
   ...
   *
   * @param {object} options The options for this Aktionenheet. Properties:
   *
   ...
   *  -  `{boolean=}` `cancelOnStateChange` Whether to cancel the Aktionenheet when navigating
   *     to a new state.  Default true.
   *  - `{string}` `cssClass` The custom CSS class name.
   *
   * @returns {function} `hideSheet` A function which, when called, hides & cancels the action sheet.
   */
  function Aktionenheet(opts) {
    ...
    function textForIcon(text) {
      if (text && /icon/.test(text)) {
        scope.$AktionenheetHasIcon = true;
      }
    }
    ...
    // removes the Aktionenheet from the screen
    ...
    scope.$deregisterBackButton = $ionicPlatform.registerBackButtonAction(
      ...
      IONIC_BACK_PRIORITY.Aktionenheet
    );
    ...
    // Expose the scope on $ionicAktionenheet's return value for the sake
    // of testing it.
...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  // history Aktionen while navigating views
  ...
  return {
    ...
    /**
     * @ngdoc method
     * @name $ionicHistory#currentTitel
...
/**
 * @ngdoc method
 * @name $ionicConfigProvider#backButton.previousTitelText
...
/**
 * @ngdoc method
 * @name $ionicConfigProvider#navBar.alignTitel
 ...
 *   <ion-nav-bar>
 *     <button ng-click="setNavTitel('banana')">
 *       Set title to banana!
 *     </button>
 *   </ion-nav-bar>
 * </body>
 ...
 *   $scope.setNavTitel = function(title) {
  ...
  return {
    $get: ['$q', '$ionicScrollDelegate', function($q, $ionicScrollDelegate) {
      var self = {
         ...
         * the registered back button Aktionen has the highest priority.
         *
         * For example, if an Aktionenheet is showing, the back button should
         * close the Aktionenheet, but it should not also go back a page view
         ...
         *
         * Your back button action will override each of the above Aktionen
         * whose priority is less than the priority you provide. For example,
         * an action assigned a priority of 101 will override the 'return to
         * previous view' action, but not any of the other Aktionen.
         *
        ...
        registerBackButtonAction: function(fn, priority, actionId) {
          ...
          self.$backButtonAktionen[action.id] = action;

          // return a function to de-register this back button action
          return function() {
            delete self.$backButtonAktionen[action.id];
          };
        },
        ...
        hardwareBackButtonClick: function(e) {
          // loop through all the registered back button Aktionen
          // and only run the last one of the highest priority
          var priorityAction, actionId;
          for (actionId in self.$backButtonAktionen) {
            if (!priorityAction || self.$backButtonAktionen[actionId].priority >= priorityAction.priority) {
              priorityAction = self.$backButtonAktionen[actionId];
            }
          }
          if (priorityAction) {
            priorityAction.fn(e);
 ...
 * - Present a list of Aktionen to perform inside one of your views
 *
 ...
 *   <ion-popover-view>
 *     <ion-header-bar>
 *       <h1 class="title">My Popover Titel</h1>
 *     </ion-header-bar>
 *     <ion-content>
 *       Hello!
 *     </ion-content>
 ...
 *
 *   // .fromTemplate() method
 *   var template = '<ion-popover-view><ion-header-bar> <h1 class="title">My Popover Titel</h1> </ion-header-bar> <ion-content> Hello! </ion-content></ion-popover-view>';
 *
  ...
  '<div class="popup-container" ng-class="cssClass">' +
    '<div class="popup">' +
      '<div class="popup-head">' +
        ...
        '<h5 class="popup-sub-title" ng-bind-html="subTitel" ng-if="subTitel"></h5>' +
      '</div>' +
 ...
 *     subTitel: 'Please use normal things',
 *     scope: $scope,
 *     buttons: [
 *       { text: 'Cancel' },
 *       {
...
function($ionicTemplateLoader, $ionicBackdrop, $q, $timeout, $rootScope, $ionicBody, $compile, $ionicPlatform, $ionicModal, IONIC_BACK_PRIORITY) {
  ...
  var $ionicPopup = {
     ...
     *   subTitel: '', // String (optional). The sub-title of the popup.
...
function($scope, $element, $attrs, $q, $ionicConfig, $ionicHistory) {
  ...
  self.title = function(newTitelText) {
    if (arguments.length && newTitelText !== titleText) {
      getEle(TITLE).innerHTML = newTitelText;
      titleText = newTitelText;
      titleTextWidth = 0;
    }
    return titleText;
  };
  ...
  self.resetBackButton = function(viewData) {
    if ($ionicConfig.backButton.previousTitelText()) {
      var previousTitelEle = getEle(PREVIOUS_TITLE);
      if (previousTitelEle) {
        previousTitelEle.classList.remove(HIDE);

        var view = (viewData && $ionicHistory.getViewById(viewData.viewId));
        var newPreviousTitelText = $ionicHistory.backTitel(view);

        if (newPreviousTitelText !== previousTitelText) {
          previousTitelText = previousTitelEle.innerHTML = newPreviousTitelText;
        }
      }
      var defaultTitelEle = getEle(DEFAULT_TITLE);
      if (defaultTitelEle) {
        defaultTitelEle.classList.remove(HIDE);
      }
    }
  };
  ...
  self.align = function(textAlign) {
    ...
    textAlign = textAlign || $attrs.alignTitel || $ionicConfig.navBar.alignTitel();

    var widths = self.calcWidths(textAlign, false);

    if (isBackShown && previousTitelText && $ionicConfig.backButton.previousTitelText()) {
      var previousTitelWidths = self.calcWidths(textAlign, true);

      var availableTitelWidth = $element[0].offsetWidth - previousTitelWidths.titleLeft - previousTitelWidths.titleRight;

      if (self.titleTextWidth() <= availableTitelWidth) {
        widths = previousTitelWidths;
      }
    }

    return self.updatePositions(titleEle, widths.titleLeft, widths.titleRight, widths.buttonsLeft, widths.buttonsRight, widths.css, widths.showPrevTitel);
  };


  self.calcWidths = function(textAlign, isPreviousTitel) {
    var titleEle = getEle(TITLE);
    ...
    var buttonsLeft = 0;
    var buttonsRight = 0;
    var isCountRightOfTitel;
    var updateTitelLeft = 0;
    var updateTitelRight = 0;
    var updateCss = '';
    ...
    if (textAlign == 'left') {
      ...
      if (buttonsLeft) {
        updateTitelLeft = buttonsLeft + 15;
      }
      if (buttonsRight) {
        updateTitelRight = buttonsRight + 15;
      }
      ...
      if (margin > 10) {
        updateTitelLeft = updateTitelRight = margin;
      }
    }
    ...
    return {
      ...
      titleLeft: updateTitelLeft,
      titleRight: updateTitelRight,
      showPrevTitel: isPreviousTitel,
      css: updateCss
    };
  };


  self.updatePositions = function(titleEle, updateTitelLeft, updateTitelRight, buttonsLeft, buttonsRight, updateCss, showPreviousTitel) {
    var deferred = $q.defer();

    // only make DOM updates when there are actual changes
    if (titleEle) {
      if (updateTitelLeft !== titleLeft) {
        titleEle.style.left = updateTitelLeft ? updateTitelLeft + 'px' : '';
        titleLeft = updateTitelLeft;
      }
      if (updateTitelRight !== titleRight) {
        titleEle.style.right = updateTitelRight ? updateTitelRight + 'px' : '';
        titleRight = updateTitelRight;
      }
    ...
    if ($ionicConfig.backButton.previousTitelText()) {
      var prevTitel = getEle(PREVIOUS_TITLE);
      var defaultTitel = getEle(DEFAULT_TITLE);

      prevTitel && prevTitel.classList[ showPreviousTitel ? 'remove' : 'add'](HIDE);
      defaultTitel && defaultTitel.classList[ showPreviousTitel ? 'add' : 'remove'](HIDE);
    }
    ...
    ionic.requestAnimationFrame(function() {
      if (titleEle && titleEle.offsetWidth + 10 < titleEle.scrollWidth) {
        ...
        updateTitelRight = testRight < minRight ? minRight : testRight;
        if (updateTitelRight !== titleRight) {
          titleEle.style.right = updateTitelRight + 'px';
          titleRight = updateTitelRight;
        }
      }
      deferred.resolve();
...
function($scope, $element, $attrs, $compile, $timeout, $ionicNavBarDelegate, $ionicConfig, $ionicHistory) {
  ...
  self.createHeaderBar = function(isActive) {
    ...
    var alignTitel = $attrs.alignTitel || $ionicConfig.navBar.alignTitel();
    var headerBarEle = jqLite('<ion-header-bar>').addClass($attrs['class']).attr('align-title', alignTitel);
    if (isDefined($attrs.noTapScroll)) headerBarEle.attr('no-tap-scroll', $attrs.noTapScroll);
    var titleEle = jqLite('<div class="title title-' + alignTitel + '">');
    var navEle = {};
    var lastViewItemEle = {};
    ...
    var headerBarInstance = {
      isActive: isActive,
      title: function(newTitelText) {
        headerBarCtrl.title(newTitelText);
      },
  ...
  self.title = function(newTitelText, headerBar) {
    if (isDefined(newTitelText)) {
      newTitelText = newTitelText || '';
      headerBar = headerBar || getOnScreenHeaderBar();
      headerBar && headerBar.title(newTitelText);
      $scope.$title = newTitelText;
      $ionicHistory.currentTitel(newTitelText);
    }
    return $scope.$title;
  };
  ...
  // DEPRECATED, as of v1.0.0-beta14 -------
  self.changeTitel = function(val) {
    deprecatedWarning('changeTitel(val)', 'title(val)');
    self.title(val);
  };
  self.setTitel = function(val) {
    deprecatedWarning('setTitel(val)', 'title(val)');
    self.title(val);
  };
  self.getTitel = function() {
    deprecatedWarning('getTitel()', 'title()');
    return self.title();
  };
  self.back = function() {
  ...
  self.getPreviousTitel = function() {
    deprecatedWarning('getPreviousTitel()', '$ionicHistory.backTitel()');
    $ionicHistory.goBack();
  };
...
function($scope, $element, $attrs, $compile, $rootScope) {
  ...
  self.beforeEnter = function(ev, transData) {
    ...
    if (transData && !transData.viewNotified) {
      ...
      viewTitel = isDefined($attrs.viewTitel) ? $attrs.viewTitel : $attrs.title;
  ...
  function afterEnter() {
    ...
    var viewTitelAttr = isDefined($attrs.viewTitel) && 'viewTitel' || isDefined($attrs.title) && 'title';
    if (viewTitelAttr) {
      titleUpdate($attrs[viewTitelAttr]);
      deregisters.push($attrs.$observe(viewTitelAttr, titleUpdate));
    }
  ...
  function titleUpdate(newTitel) {
    if (isDefined(newTitel) && newTitel !== viewTitel) {
      viewTitel = newTitel;
      navViewCtrl.title(viewTitel);
    }
  }
...
/*
 * We don't document the ionAktionenheet directive, we instead document
 * the $ionicAktionenheet service
 */
IonicModule
.directive('ionAktionenheet', ['$document', function($document) {
  return {
    restrict: 'E',
    scope: true,
    replace: true,
    ...
    template: '<div class="action-sheet-backdrop">' +
                '<div class="action-sheet-wrapper">' +
                  '<div class="action-sheet" ng-class="{\'action-sheet-has-icons\': $AktionenheetHasIcon}">' +
 ...
 * ```
 *
 * #### Grid of Bilds ([codepen](http://codepen.io/ionic/pen/5515d4efd9d66f780e96787387f41664))
 * ```html
 * <ion-content>
  ...
  function postLink(scope, element, attr, scrollCtrl, transclude) {
    ...
    function getRepeatManager() {
      return repeatManager || (repeatManager = new $ionicCollectionManager({
        ...
        widthData: widthData,
        forceRefreshBilds: !!(isDefined(attr.forceRefreshBilds) && attr.forceRefreshBilds !== 'false'),
...
function RepeatManagerFactory($rootScope, $window, $$rAF) {
  ...
  return function RepeatController(options) {
    ...
    var forceRefreshBilds = options.forceRefreshBilds;
  ...
  return {
    ...
    compile: function(tElement, tAttrs) {
      ...
      if (!hasInnerText) {
        ...
        if (!hasPreviousTitel && $ionicConfig.backButton.previousTitelText()) {
 ...
 */
IonicModule
.directive('ionNavTitel', ['$document', function($document) {
  return {
    require: '^ionNavBar',
    restrict: 'E',
    compile: function(tElement, tAttrs) {
 ...
 *
 * ```html
 * <ion-view cache-view="false" view-title="My Titel!">
 *   ...
 * </ion-view>
 * ```
 *
 ...
 * For an HTML title, such as an image, see {@link ionic.directive:ionNavTitel} instead.
