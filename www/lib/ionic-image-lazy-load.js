    ...
    function ($document, $timeout, $ionicScrollDelegate, $compile) {
        return {
            ...
            scope: {
                ...
                imageLazyBackgroundBild: "@imageLazyBackgroundBild",
                imageLazySrc: "@"
            },
            link: function ($scope, $element, $attributes) {
                ...
                function loadBild() {
                    ...
                    if ($scope.imageLazyBackgroundBild == "true") {
                        var bgImg = new Bild();
                        bgImg.onload = function () {
                            if ($attributes.imageLazyLoader) {
                                loader.remove();
                            }
                            $element[0].style.backgroundBild = 'url(' + $attributes.imageLazySrc + ')'; // set style attribute on element (it will load image)
                            if ($scope.lazyScrollResize == "true") {
                                //Call the resize to recalculate the size of the screen
                                $ionicScrollDelegate.resize();
                            }
